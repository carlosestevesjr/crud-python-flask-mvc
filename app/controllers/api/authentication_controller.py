...
itens_bp = Blueprint('itens', __name__, url_prefix='/web/itens')

# Rota para listar os itens
@itens_bp.route('/')
def index():
    items_ = Item.query.all()
    return render_template('web/itens/index.html', items=items_)
...
def create():
    ...
    return render_template('web/itens/create.html')
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        return redirect(url_for('itens.index'))
    return render_template('web/itens/update.html', item=item)
